...
internal actual class ZoneTextPrinterParser internal actual constructor(
    ...
    companion object {
        ...
        @JvmStatic
        private val cache = ConcurrentHashMap<String, SoftReference<
                MutableMap<Locale, Array<String>> >>()
    }
    ...
    private fun getDisplayName(id: String, type: Int, locale: Locale): String? {
        ...
        val ref = cache[id]
        var perLocale: MutableMap<Locale, Array<String>>? = ref?.get()
        ...
        if (null == names) {
            ...
            names[6] = TimeZoneNameUtility.retrieveGenericDisplayName(id, ZoneId.SHORT, locale)
                    ?: names[0]
            if (perLocale == null) {
                perLocale = ConcurrentHashMap()
            }
            perLocale[locale] = names
            cache[id] = SoftReference(perLocale)
        }
        when (type) {
    ...
    private val cachedTree = HashMap<Locale,Map.Entry<Int,
            SoftReference<DateTimeFormatterBuilder.Companion.PrefixTree>>>()
    ...
    override fun getTree(context: DateTimeParseContext): DateTimeFormatterBuilder.Companion.PrefixTree {
        ...
        val cached: MutableMap<Locale, Map.Entry<Int, SoftReference<DateTimeFormatterBuilder.Companion.PrefixTree>>> =
                if (isCaseSensitive) cachedTree else cachedTreeCI

        val entry: Map.Entry<Int, SoftReference<DateTimeFormatterBuilder.Companion.PrefixTree>>? = cached.get(locale)
        var tree: DateTimeFormatterBuilder.Companion.PrefixTree? = if (null == entry || entry.key != regionIdsSize) null
                else entry.value.get()
        if (null == tree) {
            ...
            cached.put(locale, SimpleImmutableEntry(regionIdsSize, SoftReference(tree)))
        }
        return tree
    }
}
